...
class CollectionPDFView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        # Logo y título
        try:
            logo_path = os.path.join(os.path.dirname(__file__), '../../static/img/logo-kosmo.png')
        ...
        collection_info_data = [
            ['Número de Cobro:', collection.payment_number or f"C-{collection.id}"],
        ...
        if collection.due_date:
            collection_info_data.append(['Fecha de Vencimiento:', collection.due_date.strftime('%d/%m/%Y')])
        
        if collection.bank:
            collection_info_data.append(['Banco:', collection.bank])
        
        if collection.nro_account:
            collection_info_data.append(['Nro. de Cuenta:', collection.nro_account])
        
        if collection.nro_operation:
            collection_info_data.append(['Nro. de Operación:', collection.nro_operation])

        # Tabla de información del cobro
        collection_table = Table(collection_info_data, colWidths=[2.5*inch, 3*inch])
        ...
        if collection_details.exists():
            ...
            for detail in collection_details:
                invoices_data.append([
                    detail.invoice.partner.business_name[:30],  # Truncar nombre largo
            ...
            # Fila de total
            invoices_data.append(['', 'TOTAL APLICADO:', f"$ {total_applied:,.2f}"])
            
            # Crear tabla de facturas
            invoices_table = Table(invoices_data, colWidths=[2.5*inch, 1.5*inch, 1.5*inch])
        ...
        if collection.processed_by or collection.approved_by:
            ...
            if additional_info:
                additional_table = Table(additional_info, colWidths=[2*inch, 3*inch])
