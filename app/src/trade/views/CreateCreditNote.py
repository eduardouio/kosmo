    ...
    def get(self, request, *args, **kwargs):
        ...
        if invoice_id:
            ...
            form.fields["invoice"].queryset = Invoice.objects.filter(
                partner=invoice.partner, is_active=True, type_document="FAC_VENTA"
            ).exclude(status="ANULADO")
            form.fields["invoice"].widget.attrs.update({"disabled": False})
        elif partner_id:

            form.fields["invoice"].queryset = Invoice.objects.filter(
                partner_id=partner_id, is_active=True, type_document="FAC_VENTA"
            ).exclude(status="ANULADO")
            form.fields["invoice"].widget.attrs.update({"disabled": False})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                if (
                    total_from_details
                    and abs(total_from_details - credit_note.amount) > 0.01
                ):
