...
class PaymentContextData(View):
    ...
    def _get_payment_statistics(self, request):
        ...
        try:
            ...
            statistics = {
                ...
                'upcoming_payments': {
                    'total_amount': float(upcoming_payments.get('total_amount', 0) or 0),
    ...
    def _calculate_payment_statistics(self, invoices_with_balance):
        ...
        try:
            ...
            loggin_event(
                'ERROR',
                f'PaymentContextData: Error calculando estadísticas: {str(e)}'
            )
            return {
    ...
    def _get_payment_configuration(self):
        ...
        try:
            ...
            loggin_event(
                'ERROR',
                f'PaymentContextData: Error obteniendo configuración: {str(e)}'
            )
            return {
                'payment_methods': [],
                'popular_banks': []
    ...
    def _handle_error(self, error, operation):
        ...
        loggin_event(
            'ERROR',
            f'PaymentContextData: Error en {operation} - {error_msg}'
        )
        ...
        return JsonResponse({
            ...
            'success': False,
            'debug_info': traceback_info if hasattr(self, 'request') and 
                         self.request.user.is_superuser else None
        }, status=500)
