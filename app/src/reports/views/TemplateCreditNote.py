...
class TemplateCreditNote(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            id_user_created = (
                credit_note.id_user_created if credit_note.id_user_created else 1
            )
            ...
            loggin_event(
                f"Nota de crédito con ID {credit_note_id} no encontrada", error=True
            )
            context["credit_note"] = None
            context["details"] = []
        except Exception as e:
            loggin_event(
                f"Error al obtener la nota de crédito " f"{credit_note_id}: {str(e)}",
                error=True,
            )
            context["credit_note"] = None
            context["details"] = []
